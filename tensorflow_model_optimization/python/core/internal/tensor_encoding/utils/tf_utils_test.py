...
class PackingUtilsTests(tf.test.TestCase, parameterized.TestCase):
  ...
  def test_pack_into_int(self, test, dtype):
    input_bitrange, expected_packed_value = test
  ...
  def test_unpack_from_int(self, test, dtype):
    original_bitrange, packed_value = test
